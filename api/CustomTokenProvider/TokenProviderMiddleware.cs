...
namespace api.CustomTokenProvider
{
    public class TokenProviderMiddleware
    {
        ...
        public LoginModel GetLoginModelRequest(HttpContext context)
        {
            ...
            var login = JsonConvert.DeserializeObject<LoginModel>(jsonString);
        ...
        public Task Invoke(HttpContext context)
        {
            ...
            if (context.Request.Path.Equals("/api/userinfo", StringComparison.Ordinal))
            {
        ...
        {
            await context.Response.WriteAsync("{\"email\": \"test.account@userinfo.com\",}");
            return;
        ...
        private async Task GenerateToken(HttpContext context)
        {
            ...
            {
                var json = GetLoginModelRequest(context);
                if (json != null)
                {
            ...
            {
                context.Response.StatusCode = 400;
                await context.Response.WriteAsync("{'erro':'Invalid username or password.'}");
                return;
            }
